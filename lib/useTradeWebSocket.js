...
import useTradeSignalSimulator from './simulateTradeSignals';
...
export function useTradeWebSocket() {
  ...
  const [isSimulationMode, setIsSimulationMode] = useState(true); // Default to simulation mode to prevent errors
  ...
  const currentUrlIndex = useRef(0); // Track which URL we're trying
  const pingTimestamp = useRef(0); // For latency tracking
  const connectionErrorShown = useRef(false); // Track if we've shown an error message
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (inSimulation) {
        ...
        console.log('Simulation or whale tracking mode detected - WebSocket will not be used');
