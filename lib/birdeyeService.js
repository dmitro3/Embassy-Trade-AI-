...
class BirdeyeService {
  ...
  /**
   * Initialize API key from MongoDB
   */
  async initializeApiKey() {
    try {
      // Get API key
      const credentials = await getApiKey('birdeye');
      
      if (credentials && credentials.api_key) {
        this.apiKey = credentials.api_key;
        logger.info('Birdeye API key loaded from MongoDB');
      } else {
        ...
        logger.warn('Using fallback Birdeye API key');
      }
    } catch (error) {
      // Fallback to environment variables or hardcoded values
      this.apiKey = process.env.NEXT_PUBLIC_BIRDEYE_API_KEY || '67f8ce614c594ab2b3efb742f8db69db';
      logger.error(`Error loading Birdeye API key: ${error.message}`);
      logger.warn('Using fallback Birdeye API key');
    }
  }
